...
```{r mean_per_day, fig.align='center'}
...
```{r mean_per_interval, fig.align='center'}
...
The 5-minute interval with the greatest average number of steps is **`r max_interval`**, with **`r format(max(interval_means$x), big.mark = ",")`** steps.
...
The above code calculates the total number of missing values in the dataset: **`r format(num_nas, big.mark = ",")`**.

```{r fill_missing_values, fig.align='center'}
...
```{r plot_filled_data, fig.align='center'}
...
The above code plots a histogram of the total number of steps taken each day using the new dataset with missing values replaced with interval averages. It also calculates the mean (**`r format(mean_steps, big.mark = ",")`**) and median (**`r format(median_steps, big.mark = ",")`**) total number of steps taken per day. 
